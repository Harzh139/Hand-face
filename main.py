...
if uploaded_file is not None:
    ...
    if image is None:
        ...
        with mp_hands.Hands(static_image_mode=True, max_num_hands=10, min_detection_confidence=0.5) as hands, \
             mp_face.FaceDetection(model_selection=1, min_detection_confidence=0.5) as face:
        ...
        st.image(cv2.cvtColor(image, cv2.COLOR_BGR2RGB), caption="\U0001F50D MediaPipe Detection", use_column_width=True)

        if face_boxes:
            st.info("\U0001F9E0 Running SAM-2 for segmentation...")
            ...
            try:
                ...
                st.image(overlay, caption="\U0001F578️ Segmented Image", use_column_width=True)

                output_path = "segmented_output.png"
                overlay.save(output_path)

                with open(output_path, "rb") as f:
                    st.download_button("\U0001F4E5 Download Segmented Image", f, file_name="segmented_output.png")
